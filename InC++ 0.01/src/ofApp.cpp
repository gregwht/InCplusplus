...
void ofApp::audioRequested 	(float * output, int bufferSize, int nChannels){
    ...
    for (int i = 0; i < bufferSize; i++){
        ...
        int b = 0; // bar number we are playing
        
        currentTime = myCounter.phasor(1, 1, beatsInBar[b]+1);
        ...
        if (timeCheck == true){
            ...
            if (currentTime >= times1[currentIndex]){
                
                // Send MIDI notes
                velocity = 64;
                note = pitch1[currentIndex];
                
                midiOut.sendNoteOn(channel, note, velocity);
                
                
                // Incrememnt current index
                if (currentIndex < arrayLength1){
                currentIndex++;
