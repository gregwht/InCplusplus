...
//--------------------------------------------------------------
void Performer::audioRequested  	(float * output, int bufferSize, int nChannels){
        ...
        if (beatCheck == true){
            ...
            if (currentTime >= time[currentBar][currentIndex]){
                ...
                if (currentIndex >= pitch[currentBar].size()){
                    ...
                    progress += beatsInBar[currentBar]; // Using beatsInBar instead of pitch[currentBar].size() because otherwise ghost notes count as full beats
