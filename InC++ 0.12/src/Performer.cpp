        ...
        if (beatCheck == true){
            ...
            if (currentTime >= time[currentBar][currentIndex]){
                ...
                if (currentIndex >= pitch[currentBar].size()){
                    ...
                    // When the whole bar has been played, add progress value
                    progress += beatsInBar[currentBar] * factor;
                    // Using beatsInBar instead of pitch[currentBar].size() because otherwise ghost notes count as full beats
                    
                    // Store the played bar as previousBar
